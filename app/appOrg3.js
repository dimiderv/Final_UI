...
async function main() {
	try {
        ...
        contractOrg3.addDiscoveryInterest({ name: chaincodeName, collectionNames: [assetCollection] });


		try {

			let randomNumber = 730//Math.floor(Math.random() * 1000) + 1;
			...
			let result = await contractOrg1.evaluateTransaction('ReadAsset', assetID);//should delete let
			console.log(`*** Result: ${prettyJSONString(result.toString())}`);
            let size = JSON.parse(result);
            var temp = result.toString();
            // console.log("This is the temp variable ", temp, " its of type ", typeof temp, " and i can read its id ", temp[0] );
            console.log("This is the size variable ", size, " its of type ", typeof size, " and i can read its id ",size.ID.length );
